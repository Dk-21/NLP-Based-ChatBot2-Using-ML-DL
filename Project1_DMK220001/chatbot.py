...
def online_search(query):
    ...
    try:
        ...
        return "Sorry, I couldn't find anything relevant to your query online."
...
def chat(user_id):
    ...
    while True:
        ...
        # Handle likes and dislikes directly in conversation
        dislikes_response = handle_likes_dislikes(user_id, user_input)
        if dislikes_response:
            print(f"Chatbot: {dislikes_response}")
            continue

        # Get response from local knowledge base or online search
        response = get_response(user_id, user_input)
        if not response or response.startswith("Sorry, I couldn't find anything"):
            response = online_search(user_input)
        
        response = preprocess_response(response)
        print(f"Chatbot: {response}")
        last_bot_response = response  # Update the last_bot_response with the current bot's response
